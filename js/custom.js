...
var loadPortfolioSinglePage = function(id, href) {
	$.ajax({
		...
		success: function(html) {
			...
			// Pause/stop video when closing portfolio
			$('#portfolio-single-'+id+' .js-close-portfolio').on('click', function() {
				// Pause HTML5 videos
				$('#portfolio-single-'+id).find('video').each(function() {
					this.pause();
					this.currentTime = 0;
				});
				// Stop iframe videos (YouTube/Vimeo)
				$('#portfolio-single-'+id).find('iframe').each(function() {
...
var owlSingleSlider = function () {
  ...
  form.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      FancyAlerts.show({
        msg: "Unable to send messages at the moment. Please try again shortly or use the contact information below to get in touch.",
        type: 'error',
  ...
  const FancyAlerts = (function () {
    const self = {};

    self.show = function (options) {
      if ($('.fancy-alert').length > 0) {
        FancyAlerts.hide();
      }
      ...
      // Show alert
      setTimeout(() => $alert.addClass('fancy-alert__active'), 10);
      setTimeout(() => $alert.addClass('fancy-alert__extended'), 500);
      ...
      $alert.on('fancyAlertClosed', () => options.onClose());
    };

    self.hide = function (delay = 0) {
      const $alert = $('.fancy-alert');

      setTimeout(() => {
        $alert.addClass("fancy-alert__removing");
        setTimeout(() => $alert.removeClass("fancy-alert__extended"), 10);
        setTimeout(() => $alert.removeClass("fancy-alert__active"), 500);
        setTimeout(() => {
